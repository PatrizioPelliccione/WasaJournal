...
\ins{We plan to evaluate the architecture framework by putting it in practice within Volvo Cars and by using ATAM (Architecture Tradeoff Analysis Method)~\cite{ATAM} as a method for evaluating our architecture with respect to identified quality attribute goals.}
...
%There are various ways of doing this, one would be to provide a collection of ADs linked together with correspondence rules to show what changes, what is constant, etc. Another way to do this is to use multiple models within a view to capture ``time slices" of the architecture evolving. There are probably other approaches as well, but these two are common. We thought about the general issue of architecture elements evolving over time, during the development of the Standard and determined there is no one solution that could be prescribed, too many variations with organizations, projects, etc.
