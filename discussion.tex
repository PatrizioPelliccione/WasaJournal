...
In this paper we describe\del{d} the current evaluation of Volvo Cars towards the definition of an architecture framework.
In this stage we are identifying potential viewpoints of the vehicles of the near future \ins{based on our shared experience in the automotive domain as well as a series of workshops for identification and validation for important concepts with a wide range of domain experts}. 
\ins{Specifically, we present challenging scenarios about future demands related to architecture and proposed three new viewpoints (Continuous Integration and Deployment, Ecosystem and Transparency, System of System).
For each of these viewpoints, we started to collect actual needs and consumers of their respective views. }
\chg{We will then}{Based on these, future work needs to} identify the \ins{most suitable} modeling languages to be used to describe the architecture.\del{. According to the actual needs and to the consumers of the particular view, we will identify the modeling language to be used}
\chg{and we will}{For these modeling languages, it is important to} find the right tradeoff between a language\ins{'s}
(i) \chg{able}{ability} to support the level of formality and precision required by disciplined development processes, or
(ii) \chg{simple}{simplicity} and \ins{it being} intuitive enough to communicate the right message to stakeholders and to promote collaboration~\cite{whatindustrywants,IEEESoftwarePatrizio}.
