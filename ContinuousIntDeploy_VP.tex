...
The viewpoint ``Continuous Integration and Deployment'' adds a development perspective to the \del{electrical}\eric{Should there not be some qualification? Should we perhaps say system architecture?} architecture and aims to give an answer to the following questions:
\begin{enumerate}
\item How do continuous integration and deployment practices in automotive engineering impact \del{electrical} architecture?
\item How do architectural decisions in the \del{electrical} architecture impact continuous integration and deployment practices?
\end{enumerate}
...
\chg{Should, for instance,}{An example for this is the question on whether it should be possible to modify or update} the software \chg{of an}{running on an} ECU responsible for a safety critical function at runtime.
...
\ins{This might be desirable, e.g. when better algorithms for autonomous driving become available.}
...
\chg{It is no use in making a good architecture if it is for the wrong purpose.}{Even a technically sound architecture is worthless if it does not fit the desired purpose.
...
\item \emph{How can we reduce the number of architectural assumptions?} The more of the architecture \chg{one make}{that is defined} up-front,  the more \ins{it is based on }assumptions \chg{are needed to be added}{instead of facts}. \del{It is hard to guarantee that these assumptions are true, moreover, if there are too many assumption one might loose the overviews as well. }\ins{This can lead to late changes, duplicate work, or in the worst case to problems that only surface once the product is on the market.}
\item \emph{How can \chg{we}{a system} respond quicker to changes in the market?} \ins{Many of the assumptions will concern the market situation once the product has been released. 
...
\ins{There are two types of dependencies, real and accidental dependencies. As cars get more and more dependent on different sensors that also increases the dependencies in the overall system. This type of dependencies needs to be handle.  One the other hand accidental dependencies should be removed.  Dependencies can hamper
\chg{team to work in isolation}{separation of work between teams}.} 

\rogardt{Dependencies seem to be one of the largest show stopper for agile devlopment}
...
Various notations have been proposed to model continuous integration and deployment pipelines \cite{example1,example2,example3}\eric{Take something from the CID SLR}.
Among those, several could be applicable to this viewpoint, e.g. \cite{Stahl}.
