...
The viewpoint ``Continuous Integration and Deployment'' adds a development perspective to the \del{electrical} architecture and aims to give an answer to the following questions:
\begin{enumerate}
...
Should, for instance, the software \chg{of an}{delivered by an} ECU responsible\eric{does an ECU responsible actually deliver software? Seems weird, as this seems to imply responsibility for a specifc ECU, while the software should be hw independent. Should it be a ``Software Responsible'' instead?}
\magnus{it's an ECU -- responsible for a safety critical\ldots not the role of ECU responsible}
for a safety critical function be modifiable at runtime?
...
\rogardt{What about models to capture dependencies}
\rogardt{Will a service oriented architecture improve CI&D, however we might leave out this discussion}
