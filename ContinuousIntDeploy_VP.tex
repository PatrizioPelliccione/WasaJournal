...
There are two types of dependencies, real \rogardt{Don't know if real is the right word here} and accidental dependencies.
\ins{An example for real dependencies is the increasing degree to that cars become dependent on different sensors, which themselves are rapidly evolving.}
\del{As cars get more and more dependent on different sensors that also increases the dependencies in the overall system.} 
This type of dependencies needs to be \ins{planned for during defining an architecture and to allow to} handle \ins{them later}.  
\ins{In contrast,} \del{One the other hand} \ins{the risk of} accidental dependencies\eric{should we give an example or definition?} should be \chg{removed}{systematically reduced and avoided}.  
\ins{Unresolved} dependencies \chg{can hamper}{require additional coordination effort between teams, which effectively slows down or even hinders continuous integration and deployment.}
