...
The viewpoint ``\Fillin{Viewpoint Name}'' focuses on the value-chain of \ugh{logical components of the electrical architecture}\eric{is this a good way of describing what is delivered by the ecosystem? It should be some abstraction of hw, software, logical components, no?} and aims at answering the following questions:
\begin{enumerate}
...
\eric{Would that be ``identify win-win situations'', identify ``development lifecycle compatibility''?}
%%%%%%%%%%
...
\eric{A bit speculative right now, no? Define transparency needs and collaboration patterns?}
...
\eric{What ever could that be? Obviously, Continuous Integration and Deployment, since if we want to do that, the syncing of development cycles becomes important. Also Systems of Systems. But in what way?}
