...
\ins{Within this paper we refer to the definition of architecture suggested by the ISO/IEC/IEEE 42010:2011 standard~\cite{42010}, which defines the architecture as: ``{\em $<$system$>$fundamental concepts} {\em or properties of a system in its environment embodied in its elements,
relationships, and in}\\ {\em the principles of its design and evolution}". Therefore, with the term architecture we are not referring to a specific view of the system and since we are focusing on the automotive domain, the architecture will refer also to physical components and their behavior in an abstract way. An architecture description is a ``{\em work product used to express an architecture}"~\cite{42010}.} 
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=\textwidth]{figures/42010}
\caption{\ins{Terms and concepts beyond the standard (figure taken from~\cite{42010})}}
\label{fig:42010}
\end{center}
\end{figure}

\ins{As shown in Figure~\ref{fig:42010},} an architecture framework is a prefabricated knowledge structure, identified by {\em architecture viewpoints}, that
...
%Focusing on the Domain-specific Architecture Framework for the Automotive Domain, t
 AAF distinguishes between two sets of architecture viewpoints and views: (i) mandatory and general viewpoints and (ii) optional viewpoints. The following viewpoints are presented according to the viewpoint catalog in~\cite{RozWooBook}.
...
The Architecture Framework for Automotive Systems (AFAS) is proposed in~\cite{Yania} through an analysis of AAF, ADF, and of Architecture Description Languages~\cite{whatindustrywants,ADL_Neno00} tailored to automotive domain, like EAST-ADL~\cite{EAST-ADL} %, TADL\footnote{TADL: Timing Augmented Description Language version
